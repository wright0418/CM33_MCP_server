 ...
 * @file     hsusbd_user.c
 * @version  V3.00
 * @brief    High Speed USB 2.0 Device(HSUSBD) driver source file
 *
...
int32_t HSUSBD_Open(S_HSUSBD_INFO_T *param, HSUSBD_CLASS_REQ pfnClassReq, HSUSBD_SET_INTERFACE_REQ pfnSetInterface)
{
    ...
    while(!(pHSUSBD->PHYCTL & HSUSBD_PHYCTL_PHYCLKSTB_Msk))
    {
        if(--u32TimeOutCnt == 0) return HSUSBD_ERR_TIMEOUT;
    }
    pHSUSBD->OPER &= ~HSUSBD_OPER_HISPDEN_Msk;   /* full-speed */
...
void HSUSBD_Start(void)
{
    ...
    pHSUSBD->OPER |= HSUSBD_OPER_HISPDEN_Msk;   /* high-speed */
    HSUSBD_CLR_SE0();
}
...
void HSUSBD_ProcessSetupPacket(void)
{
    ...
    gUsbCmd.bmRequestType = (uint8_t)(pHSUSBD->SETUP1_0 & 0xfful);
    gUsbCmd.bRequest = (uint8_t)((pHSUSBD->SETUP1_0 >> 8) & 0xfful);
    gUsbCmd.wValue = (uint16_t)pHSUSBD->SETUP3_2;
    gUsbCmd.wIndex = (uint16_t)pHSUSBD->SETUP5_4;
    gUsbCmd.wLength = (uint16_t)pHSUSBD->SETUP7_6;
        ...
        switch(gUsbCmd.bRequest)
        {
            case CLEAR_FEATURE:
            {
                if((gUsbCmd.wValue & 0xfful) == FEATURE_ENDPOINT_HALT)
                {
                    ...
                    {
                        if((((pHSUSBD->EP[i].EPCFG & 0xf0ul) >> 4) == epNum) && ((g_u32HsEpStallLock & (1ul << i)) == 0ul))
                        {
                            pHSUSBD->EP[i].EPRSPCTL = (pHSUSBD->EP[i].EPRSPCTL & 0xeful) | HSUSBD_EP_RSPCTL_TOGGLE;
                        }
                    }
                }
                /* Status stage */
    ...
    switch(gUsbCmd.bRequest)
    {
        ...
        case SET_CONFIGURATION:
        {
            if(g_hsusbd_UsbConfig == 0ul)
            {
                ...
                for(i = 0ul; i < HSUSBD_MAX_EP; i++)
                {
                    if((pHSUSBD->EP[i].EPCFG & 0x1ul) == 0x1ul)
                    {
                        pHSUSBD->EP[i].EPRSPCTL = HSUSBD_EP_RSPCTL_TOGGLE;
                    }
                }
            }
            break;
...
void HSUSBD_CtrlIn(void)
{
    ...
    if(g_hsusbd_CtrlInSize >= g_hsusbd_CtrlMaxPktSize)
    {
        ...
        for(i = 0ul; i < cnt; i++)
        {
            pHSUSBD->CEPDAT = *(uint32_t *)g_hsusbd_CtrlInPointer;
...
int32_t HSUSBD_CtrlOut(uint8_t pu8Buf[], uint32_t u32Size)
{
    ...
    while(1)
    {
        if((pHSUSBD->CEPINTSTS & HSUSBD_CEPINTSTS_RXPKIF_Msk) == HSUSBD_CEPINTSTS_RXPKIF_Msk)
        {
            for(i = 0ul; i < u32Size; i++)
            {
                pu8Buf[i] = inpb(&pHSUSBD->CEPDAT);
            }
            pHSUSBD->CEPINTSTS = HSUSBD_CEPINTSTS_RXPKIF_Msk;
            break;
        }
