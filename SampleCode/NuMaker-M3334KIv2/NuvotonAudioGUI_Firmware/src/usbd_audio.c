...
void HIDTrans_ProcessCommand(uint8_t *pu8Buffer, uint32_t u32BufferLen)
{
    if ((g_u8HIDCommandProcessing == NOT_PROCESSING) || (g_u8HIDCommandProcessing == WAITING_MORE_DATA))
    {
        ...
        if (g_sHidCmdH2D.u8Header == HIDTRANS_CHECK)
        {
            ...
            pI2CProtocolCallback = M3331_I2CCallback;
        ...
        // M3331 directly I2C command from host for I2C script mode
...
void EPC_Handler(void)
{
    ...
    uint16_t USB_BUFFER_EPC_BASE = 0, USB_BUFFER_EPC_ADD = 0, USB_BUFFER_EPC_SUB = 0;
    uint8_t *pu8buf;
...
void UAC_SetInterface(void)
{
    ...
    if (g_usbd_UsbInterface == 1) // Playback
    {
        ...
        {
            PD11 = 0; /* Set the logic value of AUDIO_JKEN as Low */
        ...
        {
            PD11 = 1; /* Set the logic value of AUDIO_JKEN as Hi */
