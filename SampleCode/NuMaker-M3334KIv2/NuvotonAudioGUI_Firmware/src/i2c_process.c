 ...
 * @file     i2c_process.c
 * @version  V1.00
 * @brief    M3331/NAU88C22 I2C process source file
 *
...
/**
 * @brief       M3331 I2C Callback Function
 * 
 * @param[in]   None
 * 
 * @return      None
 * 
 * @details     This function is used as M3331 I2C callback funtion
 */
void M3331_I2CCallback(void)
{
    uint32_t u32Status;
    ...
    else
    {
        ...
        case 0x08:                               // START has been transmitted
            ...
            {
                I2C_SET_DATA(I2C1, (0x2F << 1)); // Write SLA+W to Register I2CDAT
            }
            else
            {
                if (g_u8I2CRead)
                {
                    g_u8I2CRead = 0;
                    I2C_SET_DATA(I2C1, (g_u8SlaveAddr | 0x01)); // Write SLA+R to Register I2CDAT
                }
                else
                {
                    I2C_SET_DATA(I2C1, g_u8SlaveAddr); // Write SLA+W to Register I2CDAT
                }
            }
            I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI); // release SCL, not NACK it due to the ACK/NACK is determined by slave.
            break;
        ...
        case 0x18: // SLA+W has been transmitted and ACK has been received
            ...
            else
            {
                ...
                I2C_SET_DATA(I2C1, g_au8ToI2CDataBuf[g_u16I2CTxDataCnt++]);
                I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI); // release SCL, not NACK it due to the ACK/NACK is determined by slave.
            }
            break;
        ...
        case 0x28: // DATA has been transmitted and ACK has been received
            ...
            else // Write data finished
            {
                ...
                if (g_sHidCmdH2D.u8Header == HIDTRANS_READ)
                {
                    ...
                    I2C_SET_CONTROL_REG(I2C1, (I2C_CTL_STO | I2C_CTL_SI | I2C_CTL_STA));
                }
        ...
        case 0x30: // DATA has been transmitted and NACK has been received
            // resend data
            I2C_SET_DATA(I2C1, g_au8ToI2CDataBuf[g_u16I2CTxDataCnt - 1]);
            I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI);
            break;
        ...
        case 0x50: // DATA has been received and ACK has been returned
            g_u16I2CTimeOutCnt = 0;
            g_au8ToHIDDataBuf[g_u16I2CRxDataCnt++] = I2C_GET_DATA(I2C1);
 ...
 * @return      None
 * 
 * @details     This function is I2C1 IRQ Handler
 * 
 */
void I2C1_IRQHandler(void)
{
