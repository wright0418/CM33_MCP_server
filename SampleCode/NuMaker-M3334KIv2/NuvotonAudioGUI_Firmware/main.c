...
int main(void)
{
    ...
    printf("+------------------------------------------------------------------------+\n");
    printf("|              M3331 Codec Configuration Tool for NAU88C22               |\n");
    printf("+------------------------------------------------------------------------+\n");
    ...
    if ((g_u8HIDCommandProcessing == FINISHED) || (g_u8read_process == 1))
    {
        ...
        else if ((g_sHidCmdH2D.u8CommandType == HIDTRANS_CMD_Monitor_data)) // for new version 7802
        {
            if (g_sHidCmdH2D.u8Header == HIDTRANS_READ)
            {
                ...
                for (i = 0; i < g_u16SampleNum; i++)
                {
                    ...
                    I2C_SET_CONTROL_REG(I2C1, I2C_CTL_STA); // Start I2C transmission
        ...
        else if (g_sHidCmdH2D.u8CommandType == HIDTRANS_CMD_I2C_CLK_SET)
        {
            ...
            else // HIDTRANS_WRITE
            {
                ...
                I2C_Close(I2C1); // If there is no codec and init timeout, some wrong operation might occur
...
void HSUSBD_Init(void)
{
    ...
    if (g8_Board_mode == Board_Normal)
    {
        if (g_u32I2S0Channel == I2S_TDMCHNUM_2CH)
            gsInfo.gu8ConfigDesc = gu8ConfigDescriptor;
        else if (g_u32I2S0Channel == I2S_TDMCHNUM_4CH)
        {
            gsInfo.gu8ConfigDesc = gu8ConfigDescriptor_Normal_mode_Rec_4ch;
    ...
    // HSUSBD initial
    HSUSBD_Open(&gsInfo, UAC_ClassRequest, (HSUSBD_SET_INTERFACE_REQ)UAC_SetInterface);
 ...
 * @return      None
 *
 * @details     Reset I2C1 module, set I2C1 bus clock and enable I2C1 interrupt.
 */
void I2C1_Init(void)
{
    if (g8_Board_mode == Board_Normal)
    ...
    I2C_Open(I2C1, g_u32I2C_ClockRate);
