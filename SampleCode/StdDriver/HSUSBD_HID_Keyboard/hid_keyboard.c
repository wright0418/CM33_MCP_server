 ...
 * @file     hid_keyboard.c
 * @version  V3.00
 * @brief    HSUSBD HID keyboard sample file.
 *
...
uint8_t volatile g_u8Idle = 0;
uint8_t volatile g_u8LPMToken = 0, g_u8LPMResume = 0;
...
#ifdef SUPPORT_LPM
            HSUSBD_ENABLE_BUS_INT(HSUSBD_BUSINTEN_RSTIEN_Msk | HSUSBD_BUSINTEN_RESUMEIEN_Msk | HSUSBD_BUSINTEN_SUSPENDIEN_Msk | HSUSBD_BUSINTEN_VBUSDETIEN_Msk | HSUSBD_BUSINTEN_LPMTKNIEN_Msk);
#else
...
#ifdef SUPPORT_LPM
            HSUSBD_ENABLE_BUS_INT(HSUSBD_BUSINTEN_RSTIEN_Msk | HSUSBD_BUSINTEN_SUSPENDIEN_Msk | HSUSBD_BUSINTEN_VBUSDETIEN_Msk | HSUSBD_BUSINTEN_LPMTKNIEN_Msk);
#else
...
#ifdef SUPPORT_LPM
            HSUSBD->BUSINTEN |= HSUSBD_BUSINTEN_LPMTKNIEN_Msk;
#endif
...
#ifdef SUPPORT_LPM
        if(IrqSt & HSUSBD_BUSINTSTS_LPMTKNIF_Msk)
        {
            HSUSBD_ENABLE_BUS_INT(HSUSBD_BUSINTEN_RSTIEN_Msk | HSUSBD_BUSINTEN_RESUMEIEN_Msk | HSUSBD_BUSINTEN_SUSPENDIEN_Msk | HSUSBD_BUSINTEN_VBUSDETIEN_Msk);
            HSUSBD_CLR_BUS_INT_FLAG(HSUSBD_BUSINTSTS_LPMTKNIF_Msk);
...
void HID_InitForHighSpeed(void)
{
    ...
    HSUSBD->EP[EPA].EPRSPCTL = (HSUSBD_EP_RSPCTL_FLUSH | HSUSBD_EP_RSPCTL_MODE_AUTO);
}
...
{
    uint32_t volatile u32Key = 0xF;

    if((GPIO_GET_IN_DATA(PE) & BIT8) == 0x0)
        u32Key = 1;
    else
        u32Key = 0;
    ...
    if(u32Key == 0)
    {
        HSUSBD->EP[EPA].EPDAT = 0x00000000;
        HSUSBD->EP[EPA].EPDAT = 0x00000000;
    }
    else
    {
        HSUSBD->EP[EPA].EPDAT = 0x00040000;
        HSUSBD->EP[EPA].EPDAT = 0x00000000;
    }
}
