...
void NAU8822_Setup(void)
{
    ...
    I2C_WriteNAU8822(3,  0x07F);   /* Enable L/R main mixer, DAC */
    I2C_WriteNAU8822(4,  0x010);   /* 16-bit word length, I2S format, Stereo */
    I2C_WriteNAU8822(5,  0x001);   /* Companding control, ADC output data stream routed to DAC input data path */
    I2C_WriteNAU8822(6,  0x14D);   /* Divide by 2, 48K */
    I2C_WriteNAU8822(7,  0x000);   /* 48K for internal filter coefficients */
    I2C_WriteNAU8822(10, 0x008);   /* DAC soft mute is disabled, DAC oversampling rate is 128x */
    I2C_WriteNAU8822(14, 0x108);   /* ADC HP filter is disabled, ADC oversampling rate is 128x */
    ...
    I2C_WriteNAU8822(45, 0x03F);   /* Left channel input PGA volume control is +35.25dB */
    I2C_WriteNAU8822(47, 0x000);   /* Disable LPGA Output to LADC Mix/Boost stage input +20dB Boost */
    I2C_WriteNAU8822(50, 0x001);   /* Left DAC connected to LMIX */
    I2C_WriteNAU8822(51, 0x001);   /* Right DAC connected to RMIX */
    I2C_WriteNAU8822(70, 0x03F);   /* ALC/Noise gate threshold logic use peak-to-peak detector output value */
#endif
...
uint8_t I2C_WriteMultiByteforNAU88L25(uint8_t u8ChipAddr, uint16_t u16SubAddr, const uint8_t *p, uint32_t u32Len)
{
    ...
    /* Send device address */
    I2C_SET_DATA(I2C1, u8ChipAddr);
    I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI);
    I2C_WAIT_READY(I2C1);

    /* Send register number and MSB of data */
    I2C_SET_DATA(I2C1, (uint8_t)(u16SubAddr >> 8));
    I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI);
    I2C_WAIT_READY(I2C1);

    /* Send register number and MSB of data */
    I2C_SET_DATA(I2C1, (uint8_t)(u16SubAddr & 0x00FF));
    I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI);
    I2C_WAIT_READY(I2C1);

    /* Send data */
    I2C_SET_DATA(I2C1, p[0]);
    I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI);
    I2C_WAIT_READY(I2C1);

    /* Send data */
    I2C_SET_DATA(I2C1, p[1]);
    I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI);
    I2C_WAIT_READY(I2C1);
...
void SYS_Init(void)
{
    ...
    /* Enable I2C1 clock pin (PG2) schmitt trigger */
...
int main(void)
{
    ...
    /* Set the logic value of AUDIO_JKEN as Low */
    PD11 = 0;
