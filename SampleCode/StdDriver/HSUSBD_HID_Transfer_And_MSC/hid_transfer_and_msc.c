 ...
 * @version  V3.00
 * @brief    HSUSBD HID transfer and mass storage sample file.
 *
...
void HID_MSC_Init(void)
{
    ...
    g_TotalSectors = 60;
    g_u32MassBase = 0x20011000;
    g_u32StorageBase = 0x20012000;
}
...
void HID_MSC_ClassRequest(void)
{
    if(gUsbCmd.bmRequestType & 0x80)    /* request data transfer direction */
    {
        ...
        switch(gUsbCmd.bRequest)
        {
            case GET_MAX_LUN:
            {
                ...
                if((gUsbCmd.wValue == 0) && (gUsbCmd.wIndex == 1) && (gUsbCmd.wLength == 1))
                {
    ...
    else
    {
        ...
        switch(gUsbCmd.bRequest)
        {
            ...
            {
                /* Check interface number with cfg descriptor wIndex = interface number, check wValue = 0, wLength = 0 */
                //if((gUsbCmd.wValue == 0) && (gUsbCmd.wIndex == 1) && (gUsbCmd.wLength == 0))
                if(gUsbCmd.wIndex == 1)
                {
                    g_u8Prevent = 1;
                    /* Status stage */
...
void MSC_ProcessCmd(void)
{
    ...
    if(g_u8MscOutPacket)
    {
        ...
        {
            /* Check CBW */
            g_u32CbwSize = HSUSBD->EP[EPD].EPDATCNT & 0xffff;
            ...
            if((*(uint32_t *)(g_u32MassBase) != CBW_SIGNATURE) || (g_u32CbwSize != 31))
            {
                /* Invalid CBW */
                g_u8Prevent = 1;
                HSUSBD_SetEpStall(EPC);
                HSUSBD_SetEpStall(EPD);
                g_u32HsEpStallLock = (1 << EPC) | (1 << EPD);
                return;
            }
    ...
    while(1)
    {
        if(HSUSBD->EP[EPC].EPINTSTS & HSUSBD_EPINTSTS_BUFEMPTYIF_Msk)
        {
        ...
        {
            if((HSUSBD_GetEpStall(EPC) == 0) && (!(HSUSBD->EP[EPC].EPINTSTS & HSUSBD_EPINTSTS_BUFEMPTYIF_Msk)))
