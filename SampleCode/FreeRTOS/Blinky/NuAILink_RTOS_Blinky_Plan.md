# NuAILink RTOS Blinky 移植計畫

> 最新決策：NuAILink 已改採「重新複製 Blinky 成獨立 FreeRTOS project」的方式建立，位置為 `SampleCode/FreeRTOS/NuAILink`。第三方 cJSON 已規劃放在 `ThirdParty/cJSON`，並由新專案的 CMSIS project 直接引用 `cJSON.c` 與 `cJSON.h`。

本文件規劃如何在目前的 `SampleCode/FreeRTOS/Blinky` 專案底下，逐步建立 NuAILink SDK。規劃重點是保留 Blinky 已驗證的 FreeRTOS 啟動流程、M3331 clock/pin 初始化與 PC14 heartbeat，並把 MCP/JSON-RPC、HS USB CDC transport、BSP driver plugin framework 分層導入。

> 註解：這份文件是「實作前藍圖」。它先定義架構、檔案落點、driver/library 依賴與驗證順序，避免一開始就把 USB、RTOS、JSON、各 peripheral plugin 混在同一個 `main.c` 裡。

## 1. 目前 Blinky 專案基準

目前 Blinky 專案可作為 NuAILink 的最小 RTOS 骨架：
...
建議先在 Blinky 專案內建立 NuAILink 子目錄，不直接散落在根目錄。這樣可以保留 Blinky 原本的學習範例性，也讓後續移出成獨立 sample 時更容易。

```text
SampleCode/FreeRTOS/Blinky/
  main.c                         # 保留為 RTOS bootstrap 與 board init 入口
  FreeRTOSConfig.h               # 調整 heap、priority、diagnostics
  NuAILink_RTOS_Blinky_Plan.md   # 本規劃文件
  nualink/
    include/
...
在 `VSCode/Blinky.cproject.yml` 內建議分階段加入：
...
> 註解：若使用 root CMake 的 `standard_driver`，driver source 已幾乎全包；但目前 Blinky VSCode CMSIS project 是手動列檔，所以要在 `.cproject.yml` 明確加入 NuAILink 需要的 source。

## 9. 實作階段

### Phase 0：保護 Blinky 基準

- 確認目前 Blinky 可 build、可燒錄、PC14 可閃爍。
