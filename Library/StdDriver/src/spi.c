 ...
 * @file     spi.c
 * @version  V3.00
 * @brief    Serial Peripheral Interface(SPI) driver source file
 *
                  ...
                  uint32_t u32BusClock)
{
    ...
    if(u32MasterSlave == SPI_MASTER)
    {
        ...
        if((spi == SPI0) || (spi == SPI0_NS))
        {
            ...
            else if((CLK_GetModuleClockSource(SPI0_MODULE) << CLK_CLKSEL2_SPI0SEL_Pos) == CLK_CLKSEL2_SPI0SEL_PLL_DIV2)
            {
        ...
        else if((spi == SPI1) || (spi == SPI1_NS))
        {
            ...
            else if((CLK_GetModuleClockSource(SPI1_MODULE) << CLK_CLKSEL2_SPI1SEL_Pos) == CLK_CLKSEL2_SPI1SEL_PLL_DIV2)
            {
                u32ClkSrc = (CLK_GetPLLClockFreq() >> 1); /* Clock source is PLL/2 */
            }
        ...
        else if((spi == SPI2) || (spi == SPI2_NS))
        {
            ...
            else if((CLK_GetModuleClockSource(SPI2_MODULE) << CLK_CLKSEL2_SPI2SEL_Pos) == CLK_CLKSEL2_SPI2SEL_PLL_DIV2)
            {
                u32ClkSrc = (CLK_GetPLLClockFreq() >> 1); /* Clock source is PLL/2 */
            }
    ...
    else     /* For slave mode, force the SPI peripheral clock rate to equal APB clock rate. */
    {
        ...
        else
        {
            ...
            if((spi == SPI0) || (spi == SPI0_NS))
            {
                ...
                {
                    u32RetValue = (CLK_GetPLLClockFreq() >> 1); /* Clock source is PLL/2 */
                }
                else if((CLK_GetModuleClockSource(SPI0_MODULE) << CLK_CLKSEL2_SPI0SEL_Pos) == CLK_CLKSEL2_SPI0SEL_PCLK1)
                {
...
/**
  ...
  * @details Return the source clock frequency according to the setting of SPI0SEL (CLK_CLKSEL2[5:4]) or SPI1SEL (CLK_CLKSEL2[7:6]) or SPI2SEL (CLK_CLKSEL2[13:12]).
  */
static uint32_t SPII2S_GetSourceClockFreq(SPI_T *i2s)
{
    uint32_t u32Freq;
