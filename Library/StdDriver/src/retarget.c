...
__WEAK uint32_t ProcessHardFault(uint32_t lr, uint32_t msp, uint32_t psp)
{
    ...
    */
    ...
    /* For TrustZone */
#if defined(__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3)
    // Get SCU violation interrupt status
    if(SCU->SVINTSTS)
    {
#ifdef PDMA_VIO_TEST
        extern void dump(uint32_t u32Adqdr, uint32_t u32Size);
        extern uint32_t g_u32DestBuf;
#endif

        char *scuInt[] = {"FLASHIF", "SRAM0IF", "SRAM1IF", "SRAM2IF", "APB0IF", "APB1IF", "EBIIF", "SYSIF", "FMCIF", "PDMA0IF",
                          "CRCIF", "CANFD0IF", "CANFD1IF", "SCUIF", "GPIOIF", "HSUSBHIF", "HSUSBDIF", "SDH0IF", "CACHEIF"
                         };
        char *scuMaster[] = {"Core", "Unknown", "PDMA0", "Unknown", "SDH0", "HSUSBD", "HSUSBH", "CRC"};

        int32_t i;
        const volatile uint32_t *p32 = &SCU->FLASHVSRC;
        uint32_t u32Addr;

        for(i = 0; i < sizeof(scuInt); i++)
        {
            if(SCU->SVINTSTS & (1 << i))
            {
                u32Addr = p32[i * 2 + 1];
                printf("SCU: %s Violation, Master=%s, Addr=%x\n", scuInt[i], scuMaster[p32[i * 2]], u32Addr);
            }
        }

#ifdef PDMA_VIO_TEST
        dump(g_u32DestBuf, 32);
#endif
    }
